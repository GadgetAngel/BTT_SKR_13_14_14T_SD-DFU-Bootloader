...
## This repository can be downloaded as one large zip file from my google drive: https://drive.google.com/file/d/19VPu0yEI8OcIeZENhq5YaEITqVv6c-jt/view?usp=sharing
...
##### NOTE: I just determined that if you have the offical ST-LINK-V2 from STelectronics (not a clone) this device can connect to the board even tho the SWCLK and SWDIO lines are repurposed.  So try connecting to the SKR MINI E3 2.0 board first to see if the ST-LINK-Utility on Windows will connect, if it connects then you are good to save the bootloader, if it does not connect then I would try to load the "FW File" located in subdirectory "FW File needed to turn ON SWD connector".
